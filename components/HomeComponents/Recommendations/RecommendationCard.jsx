...
const RecommendationCard = ( props ) => {
    ...
    return (
        <CardLayout>
            <div className={`${bgColor} p-8 h-full relative card_stylings transition opacity-75`}>
                ...
                <div className={`text-xs ${textColor} italic mt-1`}>{data?.designation}</div>
                <div className={`text-sm mt-2 ${textColor} font-normal`}>{data?.view}</div>
            </div>
        </CardLayout>
    )
}
